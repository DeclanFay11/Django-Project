...
#    Vessels_in_Port.objects.all().filter(run_date=date.today()).delete()
#    return
    options = Options()
...
#    options.add_argument('User-Agent = Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Safari/537.36')
    ...
    def interceptor(request):
        del request.headers['User-Agent']  # Remember to delete the header first
        request.headers['User-Agent'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Safari/537.36'  # Spoof the referer
    web = webdriver.Chrome(options=options)
    web.request_interceptor = interceptor
...
#    print(f"body {web.last_request.response.body}")
    
    try:
        #WebDriverWait(web, 10).until(EC.presence_of_element_located((By.XPATH, '//*[@id="qc-cmp2-ui"]/div[2]/div/button[2]')))
        time.sleep(2)
    ...
    try:
        #WebDriverWait(web, 20).until(EC.presence_of_element_located((By.XPATH, '//*[@id="reporting_ag_grid"]/div/div[2]/div[3]/div/div/span')))
        time.sleep(2)
